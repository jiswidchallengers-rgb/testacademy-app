...
import { 
  ...
  usePublishPhysical,
  useGetMe,
  getListPhysicalQueryKey
} from "@workspace/api-client-react";
import type { PhysicalItem } from "@workspace/api-client-react";
...
export function Physical() {
  ...
  const { data: items, isLoading } = useListPhysical(isAdmin ? { all: "true" } : undefined);
      ...
      <div className="bg-primary text-primary-foreground py-16 mb-8 border-b-4 border-secondary">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-display font-bold uppercase tracking-tight mb-2">Physical Guidelines</h1>
              <p className="text-primary-foreground/80 max-w-2xl">
                Fitness standards and preparation guides for the physical assessment portion of the exam.
              </p>
            </div>
            {isAdmin && <PhysicalAdminDialog />}
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4">
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-white border border-dashed rounded-lg">
            ...
            <h3 className="text-lg font-medium text-primary">No physical guidelines published yet.</h3>
          </div>
        ) : (
...
function PhysicalAdminDialog({ itemToEdit }: { itemToEdit?: PhysicalItem }) {
  const [open, setOpen] = useState(false);
  const { toast } = useToast();
  const queryClient = useQueryClient();
  
  const createMut = useCreatePhysical();
  const updateMut = useUpdatePhysical();
    ...
    if (itemToEdit) {
      updateMut.mutate({ id: itemToEdit.id, data }, {
        onSuccess: () => {
          ...
          queryClient.invalidateQueries({ queryKey: getListPhysicalQueryKey({ all: "true" }) });
          setOpen(false);
        }
      });
    } else {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{itemToEdit ? "Edit Guideline" : "Add New Guideline"}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField control={form.control} name="title" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="e.g. 1 Mile Run" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
...
function AdminControls({ item }: { item: PhysicalItem }) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const delMut = useDeletePhysical();
  const pubMut = usePublishPhysical();
  ...
  return (
    <div className="flex items-center gap-1 bg-white p-1 rounded-md shadow-sm border">
      ...
      <PhysicalAdminDialog itemToEdit={item} />
