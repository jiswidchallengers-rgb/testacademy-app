...
import type { EssayItem } from "@workspace/api-client-react";
...
export function Essays() {
  ...
  const { data: items, isLoading } = useListEssays(isAdmin ? { all: "true" } : undefined);
      ...
      <div className="bg-primary text-primary-foreground py-16 mb-8 border-b-4 border-secondary">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-display font-bold uppercase tracking-tight mb-2">Essay Preparation</h1>
              <p className="text-primary-foreground/80 max-w-2xl">
                Master the structure and argumentation expected in Civil Service written examinations.
              </p>
            </div>
            {isAdmin && <EssayAdminDialog />}
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-white border border-dashed rounded-lg">
            ...
            <h3 className="text-lg font-medium text-primary">No essays published yet.</h3>
          </div>
        ) : (
              ...
              <Card key={item.id} className={`overflow-hidden border-l-4 ${item.isPublished ? 'border-l-primary' : 'border-l-muted-foreground opacity-70'}`}>
                <CardHeader className="bg-muted/30 pb-3 border-b">
                  <div className="flex justify-between items-start gap-4">
                    <CardTitle className="text-xl text-primary font-display">{item.title}</CardTitle>
                    {isAdmin && <AdminControls item={item} />}
                  </div>
                </CardHeader>
                <CardContent className="pt-6">
                  <div className="prose prose-sm md:prose-base max-w-none text-muted-foreground prose-headings:text-primary whitespace-pre-wrap">
                    {item.content}
                  </div>
                </CardContent>
              </Card>
...
function EssayAdminDialog({ itemToEdit }: { itemToEdit?: EssayItem }) {
  const [open, setOpen] = useState(false);
  const { toast } = useToast();
  const queryClient = useQueryClient();
  
  const createMut = useCreateEssay();
  const updateMut = useUpdateEssay();
    ...
    if (itemToEdit) {
      updateMut.mutate({ id: itemToEdit.id, data }, {
        onSuccess: () => {
          ...
          queryClient.invalidateQueries({ queryKey: getListEssaysQueryKey({ all: "true" }) });
          setOpen(false);
        }
      });
    } else {
      ...
      <DialogTrigger asChild>
        {itemToEdit ? (
          ...
          <Button variant="secondary" className="font-bold shadow-md">
            <Plus className="w-4 h-4 mr-2" /> Add Essay
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{itemToEdit ? "Edit Essay" : "Add New Essay"}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField control={form.control} name="title" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="e.g. Essay on Current Affairs" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField control={form.control} name="content" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Textarea className="min-h-[300px]" placeholder="Essay content..." {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
...
function AdminControls({ item }: { item: EssayItem }) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const delMut = useDeleteEssay();
  const pubMut = usePublishEssay();
  ...
  return (
    <div className="flex items-center gap-1 bg-white p-1 rounded-md shadow-sm border">
      ...
      <EssayAdminDialog itemToEdit={item} />
